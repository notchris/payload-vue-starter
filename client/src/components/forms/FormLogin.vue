    ...
    <FormKit type="form"
        ...
        <FormKit type="password"
            ...
            <template #label="context">
                <div class="flex justify-between items-center max-w-md">
                    <span :class="context.classes.label">Password</span>
                    <router-link class="text-sm hover:text-gray-500 mb-1"
        ...
        <div class="block sm:flex justify-between items-center max-w-md">
            <router-link class="hover:text-gray-500 text-sm block mb-5 sm:mb-0"
                         to="signup">New user?</router-link>
            <FormKit type="submit"
                     label="Login"
                     :classes="{
...
export default defineComponent({
    ...
    methods: {
        async doLogin(fields: { email: string, password: string }) {
            try {
                const res = await fetch(this.$apiRoot + 'api/users/login', {
                    method: 'POST',
                    headers: {
